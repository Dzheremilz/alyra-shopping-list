...
const ShoppingList = (props) => {
  ...
  const filteredShopping = shopping.filter((el) => el.trim().toLowerCase().startsWith(filter.trim().toLowerCase()))
    ...
    <>
      ...
      <ol className="list-group mb-3 shadow">
        {filteredShopping.map((product) => {
          return (<li key={product} className="list-group-item">
            <Product product={product} removeFromShoppingList={removeFromShoppingList} />
          </li>
          )
        })}
      </ol>
